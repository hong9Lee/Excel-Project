...
public class HomeController {
    ...
    @PostMapping("/upload")
    public ModelAndView upload(@RequestParam("title") String title,
                         @RequestParam("filePath") String path,
                         HttpServletRequest request,
                         Model model) throws Exception { // MultipartFile => 큰 파일을 청크 단위로 쪼개서 업로드함.
...
//            uploadService.excelUploadJdbc(uploadFile);
            uploadService.excelUploadJdbcByBulkApi(uploadFile);
            ...
            model.addAttribute("jdbc", executeTime);
            return modelAndView;
            ...
            uploadService.excelInsertByMybatis(uploadFile);
            ...
            model.addAttribute("myBatis", executeTime);
            return modelAndView;
        } else if (title.equals("jpa")) {
            long start = System.currentTimeMillis();
            uploadService.excelUploadJpa(uploadFile);

            long end = System.currentTimeMillis();
            long executeTime = (end - start) / 1000;
            System.out.println("실행시간(m) : " + executeTime);

            model.addAttribute("jpa", executeTime);
            return modelAndView;
        }
        return modelAndView;
    }
    ...
    @ResponseBody
    public Result mergeFile(String guid, String fileName) { // 최종 문서는 destTempFile를 얻는 것입니다
        String projectUrl = System.getProperty("user.dir").replaceAll("\\\\", "/");
        try {
            ...
            if (parentFileDir.isDirectory()) {
                ...
                FileUtils.deleteDirectory(parentFileDir); // 조각난 파일은 임시 디렉토리를 삭제합니다
